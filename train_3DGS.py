...
import os
# os.system('srun --mem 80G --partition=gpu --gres gpu:1 --time=8:00:00 --pty /bin/bash')
# os.system('scancel 30920368')
...
from scene import Scene, SplineScene, GaussianModel
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 1000 == 1:
                ...
                ax[1].imshow(image.cpu().detach().permute(1, 2, 0).numpy())
                ax[1].set_title(f"Splatted image ID: {viewpoint_cam.uid}")
                plt.axis('off')
                plt.show()
                print(f"total gaussians: {len(gaussians.get_xyz)}")

            # Densification
            if iteration < opt.densify_until_iter and len(gaussians.get_xyz) < 2500:
...
def spline_splatting_training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    first_iter = 0
    device = 'cuda'
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        iter_start.record()
        if iteration in [3000, 6000, 10000, 15000]:
...
def spline_training_report_single_view(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, splines, renderFunc, renderArgs):
    if tb_writer:
...
def main():
    ...
    spline_splatting_training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,
             args.checkpoint_iterations, args.start_checkpoint, args.debug_from)
    # All done
    print("\nTraining complete.")
