...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    # ply_path =
    ply_path = os.path.join(path, "point.ply")
    ...
    try:
        pcd = fetchPly(ply_path)
        # positions = transform_to_colmap(pcd.points)
        # pcd = BasicPointCloud(points=positions, colors=pcd.colors, normals=pcd.normals)
...
def transform_to_colmap(points):
    # Rotation matrix to convert Blender (Y up, Z back) to COLMAP (Y down, Z forward)
    R = np.array([
        [1, 0, 0],
        [0, 1, 0],
        [0, 0, 1]
